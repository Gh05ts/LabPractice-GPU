...
int main(int argc, char **argv)
{
	...
	const size_t in_data_bytes = sizeof(float) *padded_image_height *padded_image_width * 3;
	const size_t out_data_bytes = sizeof(float) *padded_image_height * padded_image_width;
	// 3-channel image with H/W dimensions
	float *in_h; 
	// (float*) malloc(in_data_bytes);
	cudaHostAlloc(&in_h, in_data_bytes, cudaHostAllocDefault);
	for (unsigned int i = 0; i < padded_image_height; i++)
	{
		for (unsigned int j = 0; j < padded_image_width; j++)
		{
			in_h[3 *(i *padded_image_width + j)] = rand() % 255;
			in_h[3 *(i *padded_image_width + j) + 1] = rand() % 255;
			in_h[3 *(i *padded_image_width + j) + 2] = rand() % 255;
		}
	}
	...
	cudaOccupancyMaxPotentialBlockSize(
		...
		padded_image_height*padded_image_width           // maximum threads you’ll launch
	);

	int gridSize = (padded_image_height*padded_image_width + optBlock - 1) / optBlock;
	printf("optBlock: %d\n", optBlock);
	printf("optGrid: %d\n", gridSize);
	fflush(stdout);
	...
	image2grayKernelOpt<<<gridSize, optBlock>>>(in_d, out_d, padded_image_height, padded_image_width);
